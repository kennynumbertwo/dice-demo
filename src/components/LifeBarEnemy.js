...
function LifeBarEnemy({width, current, max}) {
  const fullWidth = 38;
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={width} viewBox="0 -0.5 44 9" shapeRendering="crispEdges">
      ...
      <rect fill='#db1f1f' x={3} y={2.5} width={pixelFill} height={3} style={{transition: 'all 500ms ease-in-out'}} />
      <rect fill='#921414' x={3} y={2.5} width={pixelFill} height={1} style={{transition: 'all 500ms ease-in-out'}} />
    </svg>
  )
}

export default LifeBarEnemy;
